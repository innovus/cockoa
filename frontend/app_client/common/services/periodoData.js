(function() {
    ...
    function periodoData($http, CONFIG, autenticacion) {
        var findPeriodoActual = function() {
            return $http.get(CONFIG.http_address + '/api/todos/periodos/actual',{ headers:{Authorization : 'Bearer '+CONFIG.token} })
        };
        var findPeriodos = function() {
            return $http.get(CONFIG.http_address + '/api/todos/periodos',{ headers:{Authorization : 'Bearer '+CONFIG.token} })
        }
        var findCargasByPeriodo = function(id_periodo) {
            return $http.get(CONFIG.http_address + '/api/docentes/cargas/periodos/' + id_periodo,{ headers:{Authorization : 'Bearer '+CONFIG.token} })
        }
        return {
