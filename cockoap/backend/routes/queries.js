...
function getAllAreas(req, res, next) {
    db.any('select * from area').then(function(data) {
        res.status(200).json({
            status: 'success',
            data: data,
...
function getSingleArea(req, res, next) {
    ...
    db.one('select * from area where id_area = $1', pupID).then(function(data) {
        res.status(200).json({
            status: 'success',
            data: data,
...
function createArea(req, res, next) {
    ...
    db.none('insert into area(id_area,nombre_area) values(${id_area}, ${nombre_area})', req.body).then(function() {
        res.status(200).json({
            status: 'success',
...
function updateArea(req, res, next) {
    db.none('update area set nombre_area=$1 where id_area=$2', [req.body.nombre_area, parseInt(req.params.id)]).then(function() {
        res.status(200).json({
            status: 'success',
...
function removeArea(req, res, next) {
    var pupID = parseInt(req.params.id);
    db.result('delete from area where id_area = $1', pupID).then(function(result) {
        /* jshint ignore:start */
        res.status(200).json({
            status: 'success',
